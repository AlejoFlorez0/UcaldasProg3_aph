...
export class EliminarComponent implements OnInit {
  ...
  buscarDatos() {
    ...
    this.servicio.ObtenerUsuario(id).subscribe({
      next: (data: DatosUsuarioModel) => {
        if (data.nroDocumento && data.primerNombre && data.segundoNombre && data.primerApellido && data.segundoApellido && data.email && data.celular && data.rolId) {
  ...
  EliminarDatos() {
    this.servicio.EliminarUsuario(this.nroDocumento).subscribe({
      next: (data: any) => {
